    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-0">
      ...
      <div className="bg-white rounded-xl border border-gray-200 divide-y">
        ...
        <div className="p-4 sm:p-6">
          ...
          {/* Warning banner */}
          <div className="flex items-start gap-3 p-4 mb-4 bg-amber-50 border border-amber-200 rounded-xl text-sm">
            <AlertTriangle size={18} className="text-amber-500 mt-0.5 flex-shrink-0" />
            <p className="text-amber-800">
              Use a brand new profile, not your personal one, at your own risk.
            </p>
          </div>
          ...
          <div className="space-y-5 mb-4">
            ...
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
              <h3 className="text-sm font-semibold text-gray-900 mb-3">Facebook / Marketplace</h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Email</label>
                  <input type="email" value={accounts.facebook.email}
                         onChange={(e) => setAccounts({ ...accounts, facebook: { ...accounts.facebook, email: e.target.value } })}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPasswords.facebook ? 'text' : 'password'}
                           value={accounts.facebook.password}
                           onChange={(e) => setAccounts({ ...accounts, facebook: { ...accounts.facebook, password: e.target.value } })}
                           placeholder="Facebook password"
                           className="w-full px-4 py-2 text-sm border rounded-lg pr-10" />
                    <button type="button" onClick={() => setShowPasswords({ ...showPasswords, facebook: !showPasswords.facebook })}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                      {showPasswords.facebook ? <EyeOff size={16} /> : <Eye size={16} />}
                    </button>
                  </div>
                </div>
              </div>
            ...
            {/* Nextdoor */}
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
              <h3 className="text-sm font-semibold text-gray-900 mb-3">Nextdoor</h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Email</label>
                  <input type="email" value={accounts.nextdoor.email}
                         onChange={(e) => setAccounts({ ...accounts, nextdoor: { ...accounts.nextdoor, email: e.target.value } })}
                         placeholder="nextdoor@example.com"
                         className="w-full px-4 py-2 text-sm border rounded-lg" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Password</label>
                  <div className="relative">
                    <input type={showPasswords.nextdoor ? 'text' : 'password'}
                           value={accounts.nextdoor.password}
                           onChange={(e) => setAccounts({ ...accounts, nextdoor: { ...accounts.nextdoor, password: e.target.value } })}
                           placeholder="Nextdoor password"
                           className="w-full px-4 py-2 text-sm border rounded-lg pr-10" />
                    <button type="button" onClick={() => setShowPasswords({ ...showPasswords, nextdoor: !showPasswords.nextdoor })}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                      {showPasswords.nextdoor ? <EyeOff size={16} /> : <Eye size={16} />}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <button onClick={saveAccounts} disabled={savingAccounts}
                  className="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2">
            <Save size={16} /> {savingAccounts ? 'Saving...' : saved === 'accounts' ? 'Saved!' : 'Save Accounts'}
          </button>
        </div>
